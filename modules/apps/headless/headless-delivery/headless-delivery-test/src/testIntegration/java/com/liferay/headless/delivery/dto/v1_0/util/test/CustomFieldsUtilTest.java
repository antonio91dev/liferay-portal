...
package com.liferay.headless.delivery.dto.v1_0.util.test;
...
public class CustomFieldsUtilTest {
	...
	private <K> void _assertEquals(Map<K, ?> expectedMap, Map<K, ?> actualMap) {
		Assert.assertEquals(
		...
		for (K key : expectedMap.keySet()) {
			Assert.assertEquals(
				...
				_getString(expectedMap, key), _getString(actualMap, key));
		}
	}
	...
	private <K, V> String _getString(Map<K, V> map, K key) {
		V value = map.get(key);
		...
		if (value instanceof String) {
			return GetterUtil.getString(
				(String)value, GetterUtil.DEFAULT_STRING);
		}
		...
		if (value instanceof String[]) {
			...
			return GetterUtil.getString(array[0], GetterUtil.DEFAULT_STRING);
		}

		return GetterUtil.getString(
			String.valueOf(value), GetterUtil.DEFAULT_STRING);
	}
